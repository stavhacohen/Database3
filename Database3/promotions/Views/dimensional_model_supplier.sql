...
CREATE VIEW [promotions].[dimensional_model_supplier]
AS
...
SELECT pro.NAME Promotion_name
	...
	,sup.busines_id Supplier_Id
	,sup.name Supplier_name
	,supmat.name Suppliermatrix_segment
	...
	,fct.measure_quantity_real Quantity_sales
	 ,fct.[measure_sup_r1_promotion]
      ,fct.[measure_sup_r2_subs_promo]
      ,fct.[measure_sup_r3_subs_group]
      ,fct.[measure_sup_r4_promobuyer_existing]
      ,fct.[measure_sup_r5_promobuyer_new]
      ,fct.[measure_sup_r6_new_customer]
      ,fct.[measure_sup_r4_6_customer_effects]
      ,fct.[measure_sup_r7_product_adoption]
      ,fct.[measure_sup_r8_hoarding]
      ,fct.[measure_sup_gross_revenue_value_effect]
      ,fct.[measure_total_supplier_participation]
      ,fct.[measure_sup_netto_revenue_value_effect]
	 ,fct.[measure_kpi_Total_sellout_participation]
	 ,fct.[measure_kpi_total_sellin_discount]
	 ,fct.[measure_kpi_purchase_price]
	 ,fct.[measure_kpi_catalog_price]
	 ,fct.[measure_kpi_supplier_participation_per_product]
	,CASE 
...
FROM [promotions].[fact_performance_release] fct
...
LEFT JOIN [promotions].[dim_supplier] sup ON fct.id_supplier = sup.id_supplier
LEFT JOIN [promotions].[dim_suppliermatrix_segment] supmat ON fct.id_suppliermatrix_segment = supmat.id_suppliermatrix_segment
