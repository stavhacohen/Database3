...
SELECT pro.NAME Promotion_name
	...
	fct.id_date
FROM [promotion_dylan].[fact_performance] fct
LEFT JOIN [promotion_dylan].[dim_campaign] cam ON fct.id_campaign = cam.id_campaign
LEFT JOIN [promotion_dylan].[dim_date] dte ON fct.id_date = dte.id_date
LEFT JOIN [promotion_dylan].[dim_format] frm ON fct.id_format = frm.id_format
LEFT JOIN [promotion_dylan].[dim_place_in_store] pis ON fct.id_place_in_store = pis.id_place_in_store
LEFT JOIN [promotion_dylan].[dim_product] prd ON fct.id_product = prd.id_product
LEFT JOIN [promotion_dylan].[dim_promotion] pro ON fct.id_promotion = pro.id_promotion
LEFT JOIN [promotion_dylan].[dim_promotion_type] prot ON fct.id_promotion_type = prot.id_promotion_type
LEFT JOIN [promotion_dylan].[dim_promotion_segment] seg ON fct.id_kpi_promotion_segment = seg.id_promotion_segment
LEFT JOIN [promotion_dylan].[dim_product_category] subgrp ON fct.id_product_category = subgrp.id_product_category
LEFT JOIN [promotion_dylan].[dim_product_category] grp ON subgrp.parent_id = grp.id_product_category
LEFT JOIN [promotion_dylan].[dim_product_category] cat ON grp.parent_id = cat.id_product_category
LEFT JOIN [promotion_dylan].[dim_product_category] dep ON cat.parent_id = dep.id_product_category
LEFT JOIN [promotion_dylan].[dim_date] pstart ON fct.id_promotion_start = pstart.id_date
LEFT JOIN [promotion_dylan].[dim_date] pend ON fct.id_promotion_end = pend.id_date
LEFT JOIN [promotion_dylan].[dim_date] nydte ON fct.id_date_next_year = nydte.id_date
LEFT JOIN [promotion_dylan].[dim_date] pydte ON fct.id_date_last_year = pydte.id_date
