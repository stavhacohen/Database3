...
-- =============================================
-- Author:		Jesper de Groot X Hagai Weiss
-- Create date:	2018-06-04
-- Description:	Generates input data for RAS
-- =============================================
CREATE PROCEDURE [dbo].[HW_update_7k_ROI_promotions_input_ROI_explorer_SHALDOR]
    @start_date_expl DATE = '2016-05-01',
    @end_date_expl DATE = '2017-05-01'
AS
BEGIN
...
SELECT	  ro.PromotionNumber,
		  ...
		  --ADDED FOR SHALDOR REQUEST
		  CASE WHEN MAX(Group_name_EN) = MIN(Group_name_EN) THEN MAX(Group_name_EN) ELSE NULL END AS 'Group_name_EN',
		  CASE WHEN MAX(Group_name_HE) = MIN(Group_name_HE) THEN MAX(Group_name_HE) ELSE NULL END AS 'Group_name_HE',
		  CASE WHEN MAX(Subgroup_name_EN) = MIN(Subgroup_name_EN) THEN MAX(Subgroup_name_EN) ELSE NULL END AS 'Subgroup_name_EN',
		  CASE WHEN MAX(Subgroup_name_HE) = MIN(Subgroup_name_HE) THEN MAX(Subgroup_name_HE) ELSE NULL END AS 'Subgroup_name_HE',
		  ...
		  Multibuy_quantity,
		  -- FOR SHALDOR
		  CASE WHEN SUM(CASE WHEN ro.SourceInd = 1 THEN 1 ELSE 0 END) > 0 THEN 1 ELSE 0 END AS 'SourceInd_1',
		  CASE WHEN SUM(CASE WHEN ro.SourceInd = 2 THEN 1 ELSE 0 END) > 0 THEN 1 ELSE 0 END AS 'SourceInd_2',
...
IF OBJECT_ID('dbo.HW_SHALDOR_PG_ROI_input_ROI_explorer_promotions','U') IS NOT NULL
    DROP TABLE HW_SHALDOR_PG_ROI_input_ROI_explorer_promotions
SELECT	  ro.[PromotionNumber]
      ,[PromotionNumberUnv]
      ,[PromotionDesc]
      ,[PromotionStartDate]
	  ...
	  ,IO_indicator
		  ...
		  pr.Category_manager,
		  pr.Supplier_ID
INTO		  dbo.HW_SHALDOR_PG_ROI_input_ROI_explorer_promotions
...
IF OBJECT_ID('dbo.HW_SHALDOR_PG_ROI_input_ROI_explorer_promotion_product','U') IS NOT NULL
    DROP TABLE HW_SHALDOR_PG_ROI_input_ROI_explorer_promotion_product
SELECT	  PromotionNumber,
		  ProductNumber
INTO		  dbo.HW_SHALDOR_PG_ROI_input_ROI_explorer_promotion_product
...
IF OBJECT_ID('dbo.HW_SHALDOR_PG_ROI_input_ROI_explorer_products','U') IS NOT NULL
    DROP TABLE HW_SHALDOR_PG_ROI_input_ROI_explorer_products
SELECT	  ProductNumber,
		  Product_name_HE,
		  Category_name_HE,
		  Group_name_HE,
		  Subgroup_name_HE
INTO		  dbo.HW_SHALDOR_PG_ROI_input_ROI_explorer_products
FROM		  PG_ROI_result_product_daily
WHERE	  PromotionStartDate BETWEEN @start_date_expl AND @end_date_expl
...
IF OBJECT_ID('dbo.HW_SHALDOR_PG_ROI_input_ROI_explorer_suppliers','U') IS NOT NULL
    DROP TABLE HW_SHALDOR_PG_ROI_input_ROI_explorer_suppliers
SELECT	  Supplier_ID,
		  Supplier_name_HE
INTO		  dbo.HW_SHALDOR_PG_ROI_input_ROI_explorer_suppliers
...
DROP TABLE  [Shufersal].[dbo].[HW_SHALDOR_PG_ROI_input_ROI_explorer_groceries_promotions]
SELECT	  *
INTO		  [Shufersal].[dbo].[HW_SHALDOR_PG_ROI_input_ROI_explorer_groceries_promotions]
FROM		  [Shufersal].[dbo].[HW_SHALDOR_PG_ROI_input_ROI_explorer_promotions]
WHERE	  Department_name_EN = 'Groceries'

DROP TABLE  [Shufersal].[dbo].[HW_SHALDOR_PG_ROI_input_ROI_explorer_groceries_promotion_product]
SELECT	  *
INTO		  [Shufersal].[dbo].[HW_SHALDOR_PG_ROI_input_ROI_explorer_groceries_promotion_product]
FROM		  [Shufersal].[dbo].[HW_SHALDOR_PG_ROI_input_ROI_explorer_promotion_product]
WHERE	  PromotionNumber IN (SELECT PromotionNumber FROM [Shufersal].[dbo].[HW_SHALDOR_PG_ROI_input_ROI_explorer_groceries_promotions])

DROP TABLE  [Shufersal].[dbo].[HW_SHALDOR_PG_ROI_input_ROI_explorer_groceries_products]
SELECT	  *
INTO		  [Shufersal].[dbo].[HW_SHALDOR_PG_ROI_input_ROI_explorer_groceries_products]
FROM		  [Shufersal].[dbo].[HW_SHALDOR_PG_ROI_input_ROI_explorer_products]
WHERE	  ProductNumber IN (SELECT ProductNumber FROM [Shufersal].[dbo].[HW_SHALDOR_PG_ROI_input_ROI_explorer_groceries_promotion_product])

DROP TABLE  [Shufersal].[dbo].[HW_SHALDOR_PG_ROI_input_ROI_explorer_groceries_suppliers]
SELECT	  *
INTO		  [Shufersal].[dbo].[HW_SHALDOR_PG_ROI_input_ROI_explorer_groceries_suppliers]
FROM		  [Shufersal].[dbo].[HW_SHALDOR_PG_ROI_input_ROI_explorer_suppliers]
