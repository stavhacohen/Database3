...
-- =============================================
CREATE PROCEDURE [dbo].[update_7j_ROI_promotions_input_RAS]
    @run_nr INT = 178,
    @run_date DATE = '2019-02-04',
    @step INT = 1
AS
BEGIN
...
INSERT INTO PG_update_log
	VALUES(	@run_nr,
			...
			7,
			@step,
			'Start of [update_7j_ROI_promotions_input_RAS]',
			SYSDATETIME()
		)

IF OBJECT_ID('tempdb.dbo.#input_RAS','U') IS NOT NULL
    DROP TABLE #input_RAS
SELECT	  PromotionNumber,
		  PromotionCharacteristicsType,
		  PromotionNumberUnv,
		  PromotionDesc,
		  CampaignNumberPromo, --added as part of release 19
		  CampaignDesc,--added as part of release 19
		  PromotionStartDate,
		  PromotionEndDate,
...
IF OBJECT_ID('dbo.PG_input_RAS_LastUpdate','U') IS NOT NULL
    DROP TABLE PG_input_RAS_LastUpdate
select *
into PG_input_RAS_LastUpdate
from PG_input_RAS


IF OBJECT_ID('dbo.PG_input_RAS','U') IS NOT NULL
    DROP TABLE dbo.PG_input_RAS
SELECT [PromotionNumber]
	,[PromotionCharacteristicsType]
	 ,[PromotionNumberUnv]
	 ,[PromotionDesc]
...
INTO	   dbo.PG_input_RAS
FROM	   #input_RAS

--ReleaseX adding index tables
SET @step = @step + 1;
INSERT INTO PG_update_log
	VALUES(	@run_nr,
			@run_date,
			7,
			@step,
			'Start of updating ReleasesX tables',
			SYSDATETIME()
		)

INSERT INTO [ReleaseX_BranchID]
SELECT DISTINCT Branch_name_EN
from PG_input_RAS
Where Branch_name_EN not in (select name from [ReleaseX_BranchID])

INSERT INTO [ReleaseX_PromoType_ID]
SELECT DISTINCT Promotion_type
from PG_input_RAS
Where Promotion_type not in (select name from [ReleaseX_PromoType_ID])

INSERT INTO [ReleaseX_PlaceID]
SELECT DISTINCT Place_in_store
from PG_input_RAS
Where Place_in_store not in (select name from [ReleaseX_PlaceID])

INSERT INTO [ReleaseX_SegmentID]
SELECT DISTINCT Promotion_segment
from PG_input_RAS
Where Promotion_segment not in (select name from [ReleaseX_SegmentID])
...
INSERT INTO PG_update_log
	VALUES(	@run_nr,
			...
			7,
			@step,
			'End of [update_7j_ROI_promotions_input_RAS]',
			SYSDATETIME()
		)
