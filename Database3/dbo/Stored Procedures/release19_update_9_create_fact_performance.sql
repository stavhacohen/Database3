...
-- =============================================
-- Author:		Dylan Goldsborough & Jesper de Groot & Hagai Weiss
-- Create date: 2018-02-27
-- Description:	Creates fact_performance and dim-tables
-- =============================================
CREATE PROCEDURE [dbo].[release19_update_9_create_fact_performance]
...
DECLARE @maxdate date = (SELECT	  MAX(transactiondate)
						  FROM		  release19_PG_input_RAS)

SET IDENTITY_INSERT [promotions].[release19_dim_promotion] ON

DELETE
FROM [promotions].[release19_dim_promotion]
WHERE id_promotion <> 0

INSERT INTO [promotions].[release19_dim_promotion] (
	meta_valid_from
	,meta_valid_to
	,meta_id_update
	,meta_loaded
...
SELECT CURRENT_TIMESTAMP meta_valid_from
	...
	,a.PromotionNumber
	,a.PromotionNumberUnv--Added
	,a.folder
FROM (
	SELECT DISTINCT PromotionNumber
		,PromotionNumberUnv--Added
		,PromotionStartDate
		,folder
		,PromotionDesc
	FROM release19_PG_input_RAS WHERE TransactionDate <= @maxdate
	) a

SET IDENTITY_INSERT [promotions].[release19_dim_promotion] OFF
...
SET IDENTITY_INSERT [promotions].[release19_dim_campaign] ON

DELETE
FROM [promotions].[release19_dim_campaign]
WHERE id_campaign <> 0

INSERT INTO [promotions].[release19_dim_campaign] (
	meta_valid_from
	,meta_valid_to
	,meta_id_update
	,meta_loaded
...
FROM (
	SELECT CampaignNumberPromo--CHANGED THIS SELECT TO AVOID DUPLICATES, CHOOSING MIN DESCRIPTION
		,MIN(CampaignDesc) as CampaignDesc
	FROM release19_PG_input_RAS WHERE TransactionDate <='2018-03-12'--<= @maxdate
	GROUP BY CampaignNumberPromo
	) a

SET IDENTITY_INSERT [promotions].[release19_dim_campaign] OFF
...
SELECT SUM(revenue_value_effect)
	...
	--NOW:
	,dimcamp.id_campaign [id_campaign]
	,0 -- dummy
	,MAX([length])
	,SUM(real_quantity)
...
FROM release19_PG_input_RAS  a
LEFT JOIN promotions.dim_date dimdate ON dimdate.[date] = a.TransactionDate
LEFT JOIN promotions.release19_dim_promotion dimprom ON dimprom.business_id = a.PromotionNumber
								    AND dimprom.promotion_start = a.PromotionStartDate
-- NEW join for campaign:::::
LEFT JOIN promotions.release19_dim_campaign dimcamp ON dimcamp.business_id = a.CampaignNumberPromo
...
GROUP BY dimdate.id_date
	...
	,dimprod.id_product
	,dimformat.id_format
	,dimcamp.id_campaign -- ADDED - does order matter?
