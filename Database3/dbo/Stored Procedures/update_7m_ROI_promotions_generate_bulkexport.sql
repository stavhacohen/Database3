...
-- =============================================
CREATE PROCEDURE [dbo].[update_7m_ROI_promotions_generate_bulkexport]
	@run_nr INT = 165,
...
,case when max(PromotionStartDate)=min(PromotionStartDate) then max(PromotionStartDate) else null end as PromotionStartDate
,case when max(PromotionEndDate)=min(PromotionEndDate) then min (PromotionEndDate) else null end as PromotionEndDate
...
truncate table PG_bulkexport_weekly_drop
insert into PG_bulkexport_weekly_drop
select *
from #ras_temp
WHERE TransactionDate >= DATEADD(day,-90,@start_date) and TransactionDate <= @end_date
