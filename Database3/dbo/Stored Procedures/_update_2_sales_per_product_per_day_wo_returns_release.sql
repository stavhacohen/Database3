...
-- =============================================
CREATE PROCEDURE [dbo].[_update_2_sales_per_product_per_day_wo_returns_release]
    @run_nr INT = 1,
...
-- 180611 TC - Addition of pp
IF OBJECT_ID('tempdb.dbo.#sales_per_product_per_day_wo_returns_incl_pp_update','U') IS NOT NULL
    DROP TABLE #sales_per_product_per_day_wo_returns_incl_pp_update;
SELECT	  tt.ProductNumber,
		  tt.TransactionDate,
		  tt.SourceInd,
		  tt.Branch_name_EN,
		  tt.Number_of_customers,
		  tt.Revenue,
		  tt.Quantity,
		  tt.Margin,
		  pp.Supplier,
		  pp.Catalog_price
INTO		  #sales_per_product_per_day_wo_returns_incl_pp_update
FROM		  #sales_per_product_per_day_wo_returns_update tt
LEFT JOIN   dbo.Staging_purchase_prices2 pp
ON		  tt.TransactionDate BETWEEN pp.FROM_DATE AND pp.TO_DATE
	   AND tt.ProductNumber = pp.PRODUCT
	   AND tt.Branch_name_EN = pp.branch_name_EN
...
SELECT	  *
FROM		  #sales_per_product_per_day_wo_returns_incl_pp_update
