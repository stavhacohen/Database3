...
-- Description:	...
-- =============================================
CREATE PROCEDURE [dbo].[MM_create_Promotion_Score_Card_wave1_5]
--variables
...
left join MM_DatesConversion as M on M.convertTo=ras.TransactionDate
...
select t1.*,
		(SELECT Top 1 t2.Category_Manager_name_HE FROM #temp2 t2
...
IF OBJECT_ID('dbo.#GN_TargetViewTable_w_group_num', 'U') IS NOT NULL
    DROP TABLE dbo.GN_TargetViewTable_w_group_num;
select t1.*,
		(SELECT Top 1 t2.[Group] FROM #temp2 t2
		WHERE t2.Group_name_HE=t1.Group_name_HE
and t2.Category_name_HE=t1.Category_name_HE
		and t2.[group] is not null
		) AS Group_num
into #GN_TargetViewTable_w_group_num
...
,cats_id.cat_id
into GN_TargetViewTable_w_group_num
from #GN_TargetViewTable_w_group_num as GN
left join MM_targets_import as MM on GN.Group_num=MM.Group_id
...
--*****************************Adding time column **************************************
IF OBJECT_ID('dbo.MM_Promotion_Score_Card_wave1_5_finale_table', 'U') IS NOT NULL
    DROP TABLE dbo.MM_Promotion_Score_Card_wave1_5_finale_table;
...
into MM_Promotion_Score_Card_wave1_5_finale_table
FROM   GN_TargetViewTable_w_group_num
union 
--ADD Q2D
 SELECT Group_name_HE
      ,Category_name_HE
...
drop table #ByCategory1_5
drop table #Convert_dates_input_ras
drop table #GN_TargetViewTable_w_group_num
drop table #temp2
drop table #tempCurrentYearYTD
drop table #tempLastYearYTD
